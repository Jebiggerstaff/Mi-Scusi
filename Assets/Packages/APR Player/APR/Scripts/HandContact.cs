...
public class HandContact : MonoBehaviour
{
    ...
    SpaceScooter grabbedSpaceScooter;
    Laser grabbedLaser;
    FixedJoint joint;
    ...
    void contact(bool left, Collision col)
    {
        ...
        if (correctLayerTags(col) && APR_Player.IsKnockedOut() == false)
        {
            ...
            if (reachValue != 0 && !hasJoint && !punchingThisArm)
            {
                ...
                var l = col.gameObject.GetComponent<Laser>();
                if(l != null)
                {
                    l.grabCount++;
                    grabbedLaser = l;
                ...
                var spacescoot = col.gameObject.GetComponent<SpaceScooter>();
                if(spacescoot != null)
                {
                    spacescoot.driveCount++;
                    grabbedSpaceScooter = spacescoot;
                }
                /*
    ...
    public void UnGrab()
    {
        ...
        if(grabbedLaser != null)
        {
            grabbedLaser.grabCount--;
            grabbedLaser = null;
        }

        if (grabbedSpaceScooter != null)
        {
            grabbedSpaceScooter.driveCount--;
            grabbedSpaceScooter = null;
        }
